...
1. In Pi, invoke `openspec_relay_workflow` with operation `do` before any
   ...
   fallback, invoke its exact `openspec-relay do <change>` command. Refuse an
...
3. Let `$openspec-apply-change` own artifact loading, task implementation, and
   ...
   loop, or edit `.openspec-relay` files directly.
...
5. Route every blocking finding through the shared `planRelayChangeV1` capability.
...
7. In Pi, use `openspec_relay_workflow` operation `check` after review and
