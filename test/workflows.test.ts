...
describe('host-neutral OpenSpec Relay workflows', () => {
  it('drive Tier 0 through supported commands while OpenSpec remains planning authority', async () => {
    ...
    expect(content).toContain('openspec-relay accept');
    expect(content).toContain('openspec-relay debug');
    expect(content).toContain('openspec-relay uat');
    expect(content).toContain('tasks.md');
    expect(content).toContain('edit `.openspec-relay` files directly');
