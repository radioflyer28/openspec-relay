...
import { loadRelayConfigV2 } from '../src/config.js';
import { RelayConfigV2Schema } from '../src/schemas.js';
...
describe('Pi host adapter configuration', () => {
  it('is default-off with bounded read-only concurrency and an explicit Tier 0 rollback', () => {
    expect(RelayConfigV2Schema.parse({}).piHostAdapter).toEqual({
      enabled: false,
      forceTier0: false,
      maxReadOnlyConcurrency: 2,
    });
    expect(() => RelayConfigV2Schema.parse({ piHostAdapter: {
  ...
  it('merges project, change, and invocation settings without dropping rollback controls', async () => {
    ...
    await fs.writeFile(path.join(project.changeDir, 'relay.json'), JSON.stringify({
      piHostAdapter: { forceTier0: true },
    }));
    const config = await loadRelayConfigV2({ projectRoot: project.root, changeDir: project.changeDir });
