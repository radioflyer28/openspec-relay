...
| Cross-cutting state | one unpublished event schema; deterministic canonical replay; idempotency; corruption; orchestrator acceptance order; atomic failure recovery; projection checking; symlink and junction rejection | `test/events-v2.test.ts`, `test/state-v2.test.ts`, `test/state.test.ts`, `test/failure-injection.test.ts`, `test/gate.test.ts` | Hosted portability remains `human_needed`. |
