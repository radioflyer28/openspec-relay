...
| Release assurance | disable/configuration/discovery precedence; private pack/content/clean install without scripts; host-runner escalation; allowlisted environment and opaque output; exact candidate install and five-workflow host discovery | `test/release-assurance.test.ts`, `test/actual-candidate-install.test.ts`, `test/runner-v2.test.ts`, `test/tier0-e2e.test.ts` | Hosted Linux/Windows evidence and plugin-host discovery remain `human_needed`. |
