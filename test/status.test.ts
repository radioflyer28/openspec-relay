...
describe('canonical run status', () => {
  it('reports projection tampering as the primary blocking status in JSON and human output', async () => {
    ...
    await startGuardrailsRunV2({ change: 'demo', projectRoot: root, changedFiles: [] });
    const runPath = path.join(changeDir, '.guardrails', 'run.json');
    ...
    expect(cli.stdout).toMatch(/state integrity error/i);
