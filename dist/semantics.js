...
import { SemanticClassificationV1Schema, SemanticDowngradeV1Schema, } from './schemas.js';
const levelRank = { simple: 0, behavioral: 1, modeling: 2 };
...
function deterministicMinimum(requirement) {
    ...
    if (modeling.length > 0 || /high[- ]consequence invariant/.test(text))
        return {
            level: 'modeling',
            triggers: modeling,
    ...
    if (behavioral.length > 0 || modeling.length > 0)
        return {
            level: 'behavioral',
...
export function classifySemanticRequirements(requirements) {
...
export function reconcileSemanticClassification(planner, reviewer) {
...
export function validateSemanticStructure(options) {
    ...
    if (!hasComponentAndResponse)
        diagnostics.push(`${options.requirementId} lacks an identifiable component and required response.`);
    if (options.level === 'modeling') {
        ...
        for (const section of ['state', 'transition', 'assumption', 'proof obligation']) {
            if (!new RegExp(section, 'i').test(options.design))
                diagnostics.push(`${options.requirementId} design lacks ${section} analysis.`);
        }
