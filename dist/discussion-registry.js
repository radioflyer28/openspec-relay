...
import { z } from 'zod';
import { confirmDiscussionHandoff } from './discussion.js';
...
async function safeRegistryPath(projectRoot, create) {
    ...
    try {
        ...
        if (stat.isSymbolicLink() || !stat.isDirectory())
            throw new Error('The owned discussion registry root must be a real directory, not a symlink.');
    }
    catch (error) {
    ...
    try {
        if ((await fs.lstat(target)).isSymbolicLink())
            throw new Error('The discussion registry must not be a symlink.');
    }
    catch (error) {
...
async function readAt(filename) {
    try {
...
export async function readDiscussionRegistryV1(projectRoot) {
    const filename = await safeRegistryPath(projectRoot, false).catch((error) => {
        if (error.code === 'ENOENT')
            return discussionRegistryPath(path.resolve(projectRoot));
        throw error;
    });
    return readAt(filename);
}
async function lockedUpdate(projectRoot, update) {
    ...
    try {
        ...
        if (error.code === 'EEXIST')
            throw new Error('Concurrent discussion checkpoint replacement is in progress.');
        throw error;
    }
    try {
...
export async function selectDiscussionCheckpointV1(options) {
    ...
    if (options.workingId) {
        ...
        if (!match)
            throw new Error(`No active discussion '${options.workingId}' exists.`);
        return match;
    }
    if (active.length === 0)
        throw new Error('No active discussion checkpoint exists.');
    if (active.length > 1)
        throw new Error(`Multiple active discussions exist; select one explicitly: ${active.map((item) => item.workingId).join(', ')}.`);
    return active[0];
}
...
export async function consumeDiscussionCheckpointV1(options) {
    ...
    await lockedUpdate(options.projectRoot, async (registry) => {
        ...
        if (!current || current.fingerprint !== checkpoint.fingerprint)
            throw new Error('Discussion checkpoint changed during consumption.');
        ...
        return {
            registry: { version: 1, discussions: registry.discussions.map((item) => item.workingId === consumed.workingId ? consumed : item) },
            result: undefined,
        };
    });
    return confirmation;
}
//# sourceMappingURL=discussion-registry.js.map