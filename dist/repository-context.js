...
export function portableRepositoryPath(root, filename, pathApi = path) {
...
export async function discoverRepositoryChangedFiles(projectRoot, comparisonBase) {
    try {
        ...
        for (const candidate of bases) {
            try {
                ...
                if (!comparisonBase && head === candidateRevision)
                    continue;
                base = candidate;
                break;
            }
...
async function reference(projectRoot, filename, kind = 'repository') {
...
function resolveImportedFile(source, specifier, sourceFiles) {
    ...
    if (/\.[cm]?js$/.test(base))
        candidates.push(base.replace(/\.[cm]?js$/, '.ts'));
    return candidates.find((candidate) => sourceFiles.has(candidate));
}
function testConvention(filename) {
...
export async function compileRepositoryContext(options) {
    ...
    const getReference = async (filename, kind = 'repository') => {
    ...
    for (const source of sourceFiles) {
        ...
        for (const match of importExpressions) {
            ...
            if (!target || !changed.includes(portable(options.projectRoot, target)))
                continue;
            claims.push(claim({
    ...
    const artifact = options.compiled.artifacts.find((item) => item.kind === 'tasks');
    if (!claims.some((item) => item.category === 'implementation_analog' || item.category === 'affected_module') && artifact)
        claims.push(claim({
    ...
    const inputRevision = digest({
        ...
        claims: claims.map((item) => [item.claimId, item.evidence.map((evidence) => [evidence.referenceId, evidence.digest ?? null, evidence.available])]),
    });
    ...
    for (const item of context.claims)
        mergedClaims.set(item.claimId, item);
    const statusRank = { current: 0, stale: 1, unavailable: 2 };
...
export async function bindRepositoryEvidenceDigests(options) {
    return Promise.all(options.evidence.map(async (item) => {
        if (item.kind !== 'repository' || !item.path)
            return item;
...
export async function computeMaterialRevision(options) {
    ...
    for (const item of options.context?.claims.flatMap((claim) => claim.evidence) ?? [])
        references.set(item.referenceId, item);
    for (const item of options.evidence ?? [])
        references.set(item.referenceId, item);
    const repository = await Promise.all([...references.values()].sort((left, right) => left.referenceId.localeCompare(right.referenceId)).map(async (item) => {
        if (item.kind !== 'repository' || !item.path)
            return [item.referenceId, item.digest ?? null, item.available];
        const filename = path.resolve(options.projectRoot, ...item.path.split('/'));
        const relative = path.relative(options.projectRoot, filename);
        if (relative.startsWith('..') || path.isAbsolute(relative))
            return [item.referenceId, null, false];
        try {
            return [item.referenceId, createHash('sha256').update(await fs.readFile(filename)).digest('hex'), true];
        }
        catch {
            return [item.referenceId, null, false];
        }
