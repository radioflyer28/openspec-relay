...
import { FindingRouteV1Schema } from './schemas.js';
...
function target(finding) {
    ...
    if (/intent|product meaning|requirement omission|contradict/.test(text))
        return 'discussion';
    if (/model|feasib|counterexample|unknown technical/.test(text))
        return 'pathfinder';
    if (/plan|task coverage|assumption|compatib|verification strategy|evidence insufficient|cannot verify|verification evidence/.test(text))
        return 'planner';
    if (/re-?verify|fresh verification/.test(text))
        return 'verifier';
    return 'executor';
}
...
//# sourceMappingURL=finding-routing.js.map