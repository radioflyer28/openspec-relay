...
import { z } from 'zod';
import { FindingScopeV2Schema, PortableReferenceV2Schema, SemanticClassificationV1Schema, } from '../schemas.js';
import { PI_READ_ONLY_TOOLS, } from './host-adapter.js';
const ASSURANCE_ROLES = new Set(['plan_reviewer', 'pathfinder', 'reviewer', 'verifier']);
...
function compileRolePrompt(request, envelope, childSessionId) {
...
function parseOutput(output) {
    ...
    if (matches.length !== 1)
        throw new Error(`Expected exactly one structured result envelope; received ${matches.length}.`);
...
export function createPiRoleDispatcher(options) {
    return {
        async dispatch(request) {
            ...
            if (!request.planning)
                return errorResult('Pi assurance dispatch requires revision-bound planning context.');
            ...
            try {
                ...
                if (mismatch)
                    return errorResult(`Pi role result identity mismatch for ${mismatch[0]}.`);
                ...
                return errorResult(`Pi role result rejected: ${error.message}`);
            }
            finally {
