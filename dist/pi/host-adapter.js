...
import { z } from 'zod';
export const PI_HOST_ADAPTER_VERSION = 1;
export const SUPPORTED_PI_RANGE = '>=0.84.0 <0.85.0';
export const PI_READ_ONLY_TOOLS = Object.freeze(['find', 'grep', 'ls', 'read']);
...
function state(value, reason, remediation = []) {
...
export async function qualifyPiHostAdapter(options) {
    ...
    try {
        ...
        if (missingContracts.length)
            throw new Error(`Read-only probe lacks ${missingContracts.join(', ')}.`);
    }
    catch (error) {
        const reason = `Pi isolated-dispatch probe failed: ${error.message}`;
        return profile({
            runtime: options.runtime,
    ...
    try {
        ...
        const parallelism = concurrent === true
            ...
            : state('probe_failed', 'Pi concurrency probe did not establish safe parallel read-only dispatch.', ['Use qualified sequential dispatch or repair the concurrency adapter.']);
        ...
        return profile({
            runtime: options.runtime,
            agentDispatch,
            parallelism: state('probe_failed', `Pi concurrency probe failed: ${error.message}`, ['Use qualified sequential dispatch or repair the concurrency adapter.']),
        });
    }
}
