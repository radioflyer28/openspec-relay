...
function isNonExecutable(task) {
    const paths = task.writeSet.map((value) => value.toLowerCase());
    return paths.length > 0 && paths.every((value) => value.endsWith('.md') || value.endsWith('.txt') || value.includes('/generated/') ||
        value.endsWith('.snap') || value.endsWith('.lock'));
}
export function classifyTddRequirement(task, policy) {
    if (policy === 'off')
        return { policy, required: false, exemptionReason: 'TDD policy is off.' };
    ...
    const required = task.risk !== 'low' || task.expectedVerification.some((value) => /behavior|defect|public|contract|security|targeted-tests|risk-review/i.test(value));
    return required
...
export function validateTddEvidence(task, evidence) {
    if (!task.tddRequired)
        return { valid: true, diagnostics: [], evidenceIds: [] };
    ...
    for (const red of reds) {
        ...
        if (red.result !== 'fail' || red.exitCode === 0 || !red.relevantFailure || red.preExistingFailure)
            continue;
        if (redTime >= startTime)
            continue;
        if (!green || green.result !== 'pass' || (green.exitCode !== undefined && green.exitCode !== 0))
            continue;
        if (!refactor || refactor.result !== 'pass' ||
            (refactor.exitCode !== undefined && refactor.exitCode !== 0))
            continue;
        if (Date.parse(green.observedAt) <= redTime || Date.parse(refactor.observedAt) < Date.parse(green.observedAt))
            continue;
        if (green.sourceState === red.sourceState)
            continue;
    ...
    diagnostics.push(`Task '${task.taskId}' lacks source-bound RED, GREEN, and REFACTOR evidence for the same relevant check.`);
