...
import { ReleaseAssuranceConfigV2Schema, } from './schemas.js';
...
function hasHostReleaseRunner(runner) {
    return Boolean(runner && typeof runner.run === 'function');
}
async function runHostReleaseCommand(runner, request) {
    if (!hasHostReleaseRunner(runner))
        return undefined;
    assertReleaseCommandSafe(request.command, request.args);
    if (!request.cwd || !request.allowedRoot)
        throw new Error('Constrained candidate execution requires an allowed workspace.');
    const relative = path.relative(path.resolve(request.allowedRoot), path.resolve(request.cwd));
    if (relative.startsWith('..') || path.isAbsolute(relative))
        throw new Error('Candidate command escapes its temporary workspace.');
...
const SAFE_ENV_KEYS = ['PATH', 'SystemRoot', 'COMSPEC', 'PATHEXT', 'TMPDIR', 'TEMP', 'TMP'];
function minimalEnvironment(cwd, additions = {}) {
    const environment = {
...
function redactAndBound(value, cwd) {
...
async function manifestReference(projectRoot, filename) {
...
function candidateStatus(applicable) {
...
export async function detectReleaseApplicability(options) {
    ...
    const explicitlyEnabled = (surface) => configuredSurfaceNames.has(surface);
    ...
    try {
        ...
        const packageChanged = changed.has('package.json') || [...changed].some((file) => /(?:^|\/)(?:src|dist)\/|\.(?:[cm]?[jt]sx?)$/i.test(file));
    ...
    for (const command of config.configuredCommands)
        candidates.push(candidate({
    ...
    for (const surface of configuredSurfaceNames) {
        if (!['node_package', 'cli', 'extension', 'plugin', 'configured'].includes(surface))
            continue;
        if (candidates.some((item) => item.surface === surface))
            continue;
        candidates.push(candidate({
            candidateId: `release:configured-surface:${surface}`,
            surface: surface, applicable: true,
...
export function assertReleaseCommandSafe(command, args) {
...
function assertConfiguredReleaseCommandSafe(command, args) {
    ...
    if (interpretersAndWrappers.has(executable) || packageManagerScript) {
        throw new Error(`Configured release command '${[command, ...args].join(' ')}' uses a shell, interpreter, or indirect command wrapper ` +
...
export async function createNodePackageReleasePlan(options) {
    ...
    if (options.mode === 'full')
        commands.push({ command: 'node', args: ['-e', '/* verify configured platform and compatibility matrix */'], cwd: installDirectory });
...
function check(checkId, status, summary, evidence = []) {
    return { checkId, status, summary, evidence };
}
function candidateStatusFromChecks(checks) {
...
function mergeCandidateChecks(original, executed) {
    ...
    for (const check of executed.checks) {
        ...
        if (index < 0)
            ...
            if (precedence.indexOf(check.status) > precedence.indexOf(checks[index].status))
                checks[index] = check;
        }
    }
    return {
...
function parseNpmPack(stdout) {
    ...
    if (start < 0)
        throw new Error('npm pack did not return its JSON artifact manifest.');
    const value = JSON.parse(stdout.slice(start));
    const item = value[0];
    if (!item?.filename)
        throw new Error('npm pack did not report a candidate filename.');
    return { filename: item.filename, files: (item.files ?? []).flatMap((entry) => entry.path ? [entry.path] : []).sort() };
}
function packageDirectory(packageName, installDirectory) {
    return path.join(installDirectory, 'node_modules', ...packageName.split('/'));
}
async function productionReleasePolicyChecks(packageRoot, metadata) {
    ...
    const installDocumented = new RegExp(`(?:npm|pnpm|yarn|bun)\\s+(?:install|add)\\s+${metadata.packageName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`)
        .test(readme);
    const testedDependencyVersions = {};
    for (const dependency of Object.keys(metadata.peerDependencies)) {
        try {
            const manifest = JSON.parse(await fs.readFile(path.join(packageRoot, 'node_modules', ...dependency.split('/'), 'package.json'), 'utf8'));
            if (manifest.version)
                testedDependencyVersions[dependency] = manifest.version;
        }
        catch {
    ...
    for (const dependency of Object.keys(metadata.peerDependencies).filter((item) => !testedDependencyVersions[item])) {
        checks.push(check(`compatibility-evidence:${dependency}`, 'human_needed', `No installed ${dependency} version was available for compatibility verification.`));
    }
    return checks;
}
...
async function smokePublicEntries(packageRoot, entries, installDirectory, runner) {
    ...
    if (!smoke || smoke.exitCode !== 0)
        throw new Error('Public export smoke failed in the host release verifier.');
}
/**
...
export async function verifyNodePackageRelease(options) {
    ...
    try {
        ...
        if (metadata.buildScript && !options.buildCommand) {
            checks.push(check('build-authorization', 'human_needed', 'The package declares a build lifecycle script; configure an explicit authorized build command before candidate code executes.'));
        ...
        if (options.buildCommand) {
            ...
            if (!build) {
                checks.push(check('host-runner', 'human_needed', 'Candidate build requires an enabled host release runner.'));
                return { status: 'human_needed', checks };
            }
            checks.push(check('build', build.exitCode === 0 ? 'pass' : 'fail', build.exitCode === 0 ? 'Explicitly authorized build completed in the host runner.'
                : 'Authorized build failed in the host runner.'));
            if (build.exitCode !== 0)
                return { status: 'fail', checks };
        }
        else
            checks.push(check('build', 'pass', 'No package build lifecycle script requires execution.'));
        ...
        if (missingInstalledFiles.length)
            return { status: 'fail', artifactDigest, checks };
        for (const surface of options.manifestSurfaces ?? []) {
            ...
            if (manifestChecks.some((item) => item.status === 'fail'))
                return { status: 'fail', artifactDigest, checks };
        }
        if ((metadata.exports.length > 0 || metadata.bins.length > 0) && !hasHostReleaseRunner(options.releaseRunner)) {
            checks.push(check('public-smoke', 'human_needed', 'Executing installed candidate exports or CLIs requires an enabled host release runner.', artifactEvidence));
            return { status: 'human_needed', artifactDigest, checks };
        }
        try {
            if (metadata.exports.length)
                await smokePublicEntries(installedRoot, metadata.exports, installDirectory, options.releaseRunner);
            for (const bin of metadata.bins) {
                ...
                if (!smoke || smoke.exitCode !== 0)
                    throw new Error(`CLI '${bin}' failed in the host release verifier.`);
            }
        ...
        if (options.mode === 'full')
            checks.push(check('platform-matrix', 'human_needed', 'Full-mode cross-platform evidence is collected by hosted CI rather than inferred locally.', artifactEvidence));
...
export async function runConfiguredReleaseCommand(options) {
    ...
    const evidence = [{ referenceId: `config:release-command:${options.configuredCommand.id}`, kind: 'external', externalId: options.configuredCommand.id, available: true }];
    try {
        if (!hasHostReleaseRunner(options.releaseRunner))
            return candidate({
                ...
                checks: [check(`configured:${options.configuredCommand.id}`, 'human_needed', 'Configured candidate code requires an enabled host release runner.', evidence)],
            });
        ...
        return candidate({
            ...
            activationEvidence: evidence,
            status,
            checks: [check(`configured:${options.configuredCommand.id}`, status, status === 'pass' ? `Configured command '${options.configuredCommand.id}' completed in a temporary workspace.`
...
async function verifyManifestSurface(options) {
    ...
    try {
        const raw = JSON.parse(await fs.readFile(path.join(options.packageRoot, filename), 'utf8'));
        if (!(raw.id ?? raw.name) || !raw.version)
            throw new Error('manifest requires an identity and version.');
        ...
        if (missingEntries.length)
            throw new Error(`manifest references missing workflow entries: ${missingEntries.join(', ')}`);
        ...
        if (options.surface === 'plugin') {
            checks.push(check('plugin-host-discovery', 'human_needed', 'A compatible Codex host session is required to prove installed plugin discovery and public behavior.'));
            return checks;
        }
        ...
        if (!await exists(coreCli)) {
            checks.push(check('extension-host-discovery', 'human_needed', 'An installed OpenSpec host CLI is required to prove extension discovery.'));
            return checks;
        }
        ...
        return [check(`${options.surface}-manifest`, 'fail', `${options.surface} manifest conformance failed: ${error.message}`)];
    }
}
...
export async function executeReleaseCandidates(options) {
    ...
    for (const item of options.candidates) {
        ...
        if (item.surface === 'configured') {
            ...
            const executed = configuredCommand
        ...
        const platformChecks = (options.config.requiredPlatforms ?? []).map((required) => check(`platform:${required}`, required === platform ? 'pass' : 'human_needed', required === platform
...
export async function hashReleaseArtifact(filename) {
    return createHash('sha256').update(await fs.readFile(filename)).digest('hex');
}
export async function inspectNodePackageMetadata(packageRoot) {
    const manifest = JSON.parse(await fs.readFile(path.join(packageRoot, 'package.json'), 'utf8'));
    if (!manifest.name || !manifest.version)
        throw new Error('Node package verification requires package name and version.');
...
export function selectReleaseChecks(mode) {
    const checks = ['pack', 'content', 'clean-install', 'public-smoke', 'metadata'];
    if (mode === 'full')
        checks.push('platform-matrix', 'compatibility-matrix');
    return checks;
}
function versionInRange(version, range) {
    if (!valid(version) || !validRange(range))
...
export function createConfiguredCommandPlan(options, authorization = 'configured-distribution') {
    if (authorization === 'explicit-build')
        assertReleaseCommandSafe(options.command, options.args);
    else
        assertConfiguredReleaseCommandSafe(options.command, options.args);
...
//# sourceMappingURL=release-assurance.js.map