...
export const RELAY_V1_STATE_VERSION = 1;
...
export const ArtifactReferenceV1Schema = z.object({
    ...
    path: z.string().min(1).refine((value) => !/^(?:[A-Za-z]:[\\/]|[\\/])/.test(value) &&
        !value.split('/').includes('..') && !value.includes('\\'), 'artifact path must be a contained portable change-relative path'),
...
export const EvidenceV1Schema = z.object({
    ...
    sourceDigests: z.record(z.string().min(1), z.string().regex(/^[a-f0-9]{64}$/)).optional(),
...
export const PortableReferenceV2Schema = z.object({
    ...
    path: z.string().min(1).refine((value) => !/^(?:[A-Za-z]:[\\/]|[\\/])/.test(value) &&
        !value.split('/').includes('..') && !value.includes('\\'), 'reference path must be a portable project-relative identity').optional(),
...
export const ConfiguredReleaseCommandV2Schema = z.object({
    ...
    expectedArtifacts: z.array(z.string().min(1).refine((value) => !/^(?:[A-Za-z]:[\\/]|[\\/])/.test(value) &&
        value.split('/').every((segment) => segment.length > 0 && segment !== '.' && segment !== '..') &&
        !value.includes('\\'), 'expected artifact path must be a portable relative path inside the temporary release workspace')).max(50).default([]),
...
export const DebugVerificationV2Schema = z.object({
    ...
    if (Boolean(value.findingId) === Boolean(value.checkId))
        context.addIssue({
    ...
    if (!regressionPair && !value.exemption)
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: 'Debug verification requires distinct fail-before and pass-after evidence or an accepted exemption.',
        });
    if (regressionPair && value.failBeforeEvidence.digest === value.passAfterEvidence.digest)
        context.addIssue({
            code: z.ZodIssueCode.custom,
...
const PortableCheckpointPathSchema = z.string().min(1).max(1024).refine((value) => !/^(?:[A-Za-z]:[\\/]|[\\/])/.test(value) &&
    value.split('/').every((segment) => segment.length > 0 && segment !== '.' && segment !== '..') &&
    !value.includes('\\'), 'checkpoint path must be a contained portable project-relative path');
