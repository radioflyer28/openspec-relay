...
import { AssuranceCheckV2Schema, RelayAssuranceV2Schema, RelayConfigV1Schema, RelayEventEnvelopeV2Schema, RelayEventPayloadV2Schema, RelayEventStoreV2Schema, RelayRunV2Schema, } from './schemas.js';
import { assuranceStatePath, atomicWriteRelayJson, digestJson, relayGeneratedPath, readRelayText, runStatePath, } from './state.js';
export function eventStorePath(changeDir) {
    return relayGeneratedPath(changeDir, 'events');
}
function eventStoreV2(value) {
    const store = RelayEventStoreV2Schema.parse(value);
    const ids = new Set();
    for (const event of store.events) {
        ...
        if (ids.has(event.eventId))
            throw new Error(`Event store contains duplicate event ID '${event.eventId}'.`);
        ids.add(event.eventId);
    }
    return store;
}
export function createRelayEventV2(options) {
    const payload = RelayEventPayloadV2Schema.parse(options.payload);
    return RelayEventEnvelopeV2Schema.parse({
        version: 2,
        ...options,
        payload,
...
export async function readEventStoreV2(changeDir) {
    return eventStoreV2(JSON.parse(await readRelayText(changeDir, eventStorePath(changeDir))));
}
...
export async function readCanonicalEventStore(changeDir) {
    try {
        return await readEventStoreV2(changeDir);
    }
    catch (error) {
        throw new Error(`OpenSpec Relay execution records are missing or unsupported. Remove the change's .openspec-relay directory ` +
            `and start a new run to regenerate it: ${error.message}`);
    }
}
export async function appendRelayEventV2(options) {
    const event = RelayEventEnvelopeV2Schema.parse(options.event);
    if (event.payloadDigest !== digestJson(event.payload)) {
    ...
    await atomicWriteRelayJson(options.changeDir, eventStorePath(options.changeDir), next, {
...
function findingFromV1(finding, event) {
...
function assuranceStatusV2(options) {
    if (options.checks.some((check) => check.status === 'error') ||
        options.releaseCandidates.some((candidate) => candidate.status === 'error'))
        return 'error';
    if (options.checks.some((check) => check.status === 'fail') ||
        options.releaseCandidates.some((candidate) => candidate.status === 'fail') ||
        evaluateFindingObligations({ findings: options.findings, scenarios: options.uatScenarios }).blocking.length > 0)
        return 'fail';
    if (options.planStale)
        return 'fail';
    if (options.semanticDowngrades.some((downgrade) => downgrade.status === 'human_needed'))
        return 'human_needed';
    if (options.debugSessions.some((session) => session.status !== 'resolved' || !session.verification))
        return 'human_needed';
    if (options.uatScenarios.some((scenario) => ['awaiting_human', 'blocked', 'stale'].includes(scenario.status)) ||
        options.releaseCandidates.some((candidate) => candidate.status === 'human_needed'))
        return 'human_needed';
    if (options.checks.some((check) => check.status === 'pending'))
        return 'pending';
    if (options.checks.some((check) => check.status === 'warn'))
        return 'warn';
    return 'pass';
}
export function replayRelayEventsV2(options) {
    const store = eventStoreV2(options.store);
    const legacyConfigInput = Object.fromEntries(Object.entries(store.seed.config)
        .filter(([key]) => key !== 'features' && key !== 'piHostAdapter'));
    const legacyConfig = RelayConfigV1Schema.parse({ ...legacyConfigInput, version: 1 });
    ...
    const uatScenarios = new Map();
    const releaseCandidates = new Map();
    const semanticClassifications = new Map();
    const semanticDowngrades = new Map();
    const pathfinderResults = new Map();
    ...
    const applyV1Payload = (payload, event) => {
        ...
        else if (payload.type === 'evidence.recorded')
            evidence.push(payload.evidence);
        else if (payload.type === 'finding.recorded')
            findings.set(payload.finding.findingId, findingFromV1(payload.finding, event));
        else if (payload.type === 'repair.recorded')
            repairs.push(payload.repair);
        else if (payload.type === 'deviation.recorded')
            deviations.push(payload.deviation);
        else if (payload.type === 'human.decision') {
            const label = `${payload.gateId}: ${payload.reason ?? `human ${payload.decision}`}`;
            if (payload.decision === 'requested')
                humanActions.push(label);
            else {
                const index = humanActions.findIndex((item) => item.includes(payload.gateId));
                if (index >= 0)
                    humanActions.splice(index, 1);
            }
        }
    };
    for (const event of store.events) {
        ...
        if (['task.transition', 'evidence.recorded', 'finding.recorded', 'deviation.recorded',
            ...
            applyV1Payload(payload, event);
        }
        else if (payload.type === 'host.adapter_qualified')
            hostAdapter = payload.adapter;
        else if (payload.type === 'context.compiled')
            repositoryContext = payload.context;
        ...
        else if (payload.type === 'readiness.evaluated')
            readiness = payload.result;
        ...
        else if (payload.type === 'finding.discovered')
            findings.set(payload.finding.findingId, payload.finding);
        ...
        else if (payload.type === 'debug.session_started')
            debugSessions.set(payload.session.sessionId, payload.session);
        else if (payload.type === 'debug.hypothesis_recorded') {
            const session = debugSessions.get(payload.sessionId);
            if (session)
                debugSessions.set(payload.sessionId, { ...session,
        ...
        else if (payload.type === 'uat.scenario_recorded')
            uatScenarios.set(payload.scenario.scenarioId, payload.scenario);
        else if (payload.type === 'uat.scenario_retest') {
            const scenario = uatScenarios.get(payload.scenarioId);
            if (scenario) {
                const updated = { ...scenario, status: 'awaiting_retest', sourceRevision: payload.sourceRevision };
            ...
            const scenario = uatScenarios.get(payload.scenarioId);
            if (scenario)
                uatScenarios.set(payload.scenarioId, { ...scenario, status: 'stale', sourceRevision: payload.sourceRevision });
        }
        else if (payload.type === 'scenario.coverage_reconciled')
            scenarioCoverage = payload.coverage;
        else if (payload.type === 'uat.disposition_recorded') {
            const scenario = uatScenarios.get(payload.scenarioId);
            if (scenario)
                uatScenarios.set(payload.scenarioId, { ...scenario, status: payload.status,
                    sourceRevision: payload.sourceRevision,
                    disposition: { actor: payload.actor, recordedAt: event.occurredAt, notes: payload.notes, evidence: payload.evidence } });
        }
        else if (payload.type === 'release.evaluated')
            releaseCandidates.set(payload.candidate.candidateId, payload.candidate);
        else if (payload.type === 'checks.evaluated')
            checks = payload.checks.map((check) => AssuranceCheckV2Schema.parse(check));
        else if (payload.type === 'run.status_updated')
    ...
    const staleEvidenceIds = evidence.filter((item) => Object.entries(item.sourceDigests ?? {}).some(([artifactPath, digest]) => currentDigests.get(artifactPath) !== digest)).map((item) => item.evidenceId).sort();
    ...
    const assurance = RelayAssuranceV2Schema.parse({
        version: 2,
        runId: store.runId,
    ...
    const run = RelayRunV2Schema.parse({
        version: 2,
        runId: store.runId,
        changeName: store.changeName,
...
export async function writeReplayedProjectionsV2(options) {
    const projection = replayRelayEventsV2(options);
    await atomicWriteRelayJson(options.changeDir, assuranceStatePath(options.changeDir), projection.assurance);
    await atomicWriteRelayJson(options.changeDir, runStatePath(options.changeDir), projection.run);
    return projection;
}
