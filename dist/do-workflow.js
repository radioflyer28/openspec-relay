...
import { dispatchRoleV2 } from './execution-adapters.js';
...
import { recordDispatchedRoleResultV2, transitionFindingV2, verifyFindingFromDispatchedResultV2, } from './v2-operations.js';
function stableFindingIds(result) {
...
async function currentApproved(options) {
...
export async function assertCurrentPlanApprovalV1(options) {
...
async function setRunStatus(changeDir, status, origin) {
...
async function taskIsComplete(changeDir, taskId, taskMetadata) {
...
export async function doRelayChangeV1(options) {
    ...
    try {
        ...
        const resolved = await resolveChangeDirectory(options);
        const canonical = await loadCanonicalRelayState(resolved.changeDir);
        return { status: 'human_needed', summary: error.message,
    ...
    while (convergenceCycles < 2) {
        ...
        for (const task of selected) {
            ...
            if (request.action === 'repair')
                cycleRepairEvidence.push(...(result.evidence ?? []));
            ...
            if (!(await taskIsComplete(current.resolved.changeDir, task.taskId, current.canonical.store.seed.config.taskOverrides))) {
        ...
        for (const findingId of cycleFindingIds)
        ...
        const revisedPlanFindingIds = current.canonical.projection.assurance.findingRoutes
            ...
            .filter((findingId) => current.canonical.projection.assurance.findings.some((finding) => finding.findingId === findingId && finding.state === 'open'));
        for (const findingId of revisedPlanFindingIds)
            await transitionFindingV2({
                change: options.change, projectRoot: current.resolved.projectRoot, findingId, action: 'repair',
        ...
        const roleContext = {
            ...
            invocation: 'do_replan',
            artifactRefs: current.canonical.compiled.artifacts.map((item) => `${current.resolved.changeRef}/${item.path}`),
        ...
        if (!failed) {
            ...
            if (verificationReceipt.result.status === 'pass')
                for (const findingId of verificationFindingIds) {
        ...
        if (!failed) {
            ...
            if (checked.assurance.status === 'pass' || checked.assurance.status === 'warn')
                return {
...
//# sourceMappingURL=do-workflow.js.map