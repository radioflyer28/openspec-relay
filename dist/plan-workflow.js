...
import { PathfinderResultV1Schema, PlanReviewResultV1Schema, } from './schemas.js';
import { classifySemanticRequirements, reconcileSemanticClassification, resolveSemanticClassification, validateSemanticStructure, } from './semantics.js';
...
function reviewFindingIds(result) {
...
function mergePlannerClassifications(options) {
    if (!options.supplemental?.length)
...
export async function planRelayChangeV1(options) {
    const resolved = await resolveChangeDirectory({ projectRoot: options.projectRoot, change: options.change });
    if (resolved.archived)
        throw new Error(`Cannot plan archived change '${resolved.changeName}'.`);
        ...
        requests: pathfinderQuestions.map((question) => {
            ...
            return { id: pathfinderId, prerequisites: [], run: async () => {
                    ...
                    try {
                        ...
                        if (!receipt.result.pathfinder)
                            throw new Error('Pathfinder dispatch omitted its structured planning result.');
    ...
    while (assuranceDispatcher && cycles < 2) {
        ...
        if (review.status === 'pass' || review.status === 'human_needed' || review.status === 'error')
            break;
        const signature = findingIds.join('|') || receipt.result.summary;
        if (signature === priorBlocking || cycles === 2)
            break;
    ...
    const semanticDiagnostics = classifications.flatMap((classification) => {
        ...
        if (!requirement)
            return [`${classification.requirementId} is missing from the compiled OpenSpec requirements.`];
