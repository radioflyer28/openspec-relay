...
export const relayAssuranceGate: GateProviderV1 = {
  async evaluate(context) {
    try {
      ...
      if (!canonical.projectionsMatch) {
        return result(
          'error',
          'OpenSpec Relay projections do not match canonical event replay.',
          [`${path.join('.openspec-relay', 'events.json')}#sha256=${canonicalRevision}`],
          ['Run openspec-relay check to regenerate projections from canonical events.'],
        );
      }
      if (run.runId !== assurance.runId || run.changeName !== context.changeName ||
        ...
        return result(
          'error',
          'OpenSpec Relay run, assurance, and OpenSpec change identities do not match.',
          [],
          ['Run openspec-relay check to reconcile generated execution records.'],
        );
      }
      ...
      if (run.assuranceDigest !== assuranceDigest) {
        return result(
          'error',
          'OpenSpec Relay assurance record does not match the digest bound to the run.',
          evidence,
          ['Run openspec-relay check to regenerate matching run and assurance records.'],
        );
      }
      {
        ...
        if (findings.blocking.length > 0) return result(
          'fail',
          `OpenSpec Relay has unresolved blocking findings: ${findings.blocking.join(', ')}.`,
          evidence,
        ...
        if (debugging.length > 0) return result(
          'human_needed',
          `OpenSpec Relay has unresolved debugging investigations: ${debugging.map((session) =>
        ...
        if (uat.blocking.length > 0) return result(
          'human_needed',
          `OpenSpec Relay requires UAT action for: ${uat.blocking.join(', ')}.`,
          evidence,
          ['Run openspec-relay uat and record an explicit human disposition for each scenario.'],
        );
      }
      if (assurance.status === 'human_needed' || assurance.unresolvedHumanActions.length > 0) {
        return result(
          'human_needed',
          assurance.unresolvedHumanActions.join('; ') || 'OpenSpec Relay requires human acceptance.',
          evidence,
      ...
      if (assurance.status === 'pending') {
        return result(
          'fail',
          'OpenSpec Relay assurance is incomplete.',
          evidence,
          ['Run openspec-relay check and resolve pending checks.'],
        );
      }
      if (assurance.status === 'fail' || assurance.status === 'error') {
        ...
        return result(
          assurance.status,
          `OpenSpec Relay assurance is ${assurance.status}${blocking.length ? `: ${blocking.join(', ')}` : '.'}`,
          evidence,
          ['Resolve the blocking checks and rerun openspec-relay check.'],
        );
      }
      ...
      return result(
        'error',
        `OpenSpec Relay assurance records are unavailable or invalid: ${(error as Error).message}`,
        [],
        ['Run openspec-relay plan or check to recreate valid generated records.'],
      );
    }
  },
};
