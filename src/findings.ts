...
export function createFindingId(input: Pick<FindingDiscoveryInputV2,
  'providerId' | 'ruleId' | 'category' | 'scope'>): string {
...
export function discoverFinding(input: FindingDiscoveryInputV2): FindingLifecycleRecordV2 {
...
function assertAuthorizedTransition(options: {
  ...
  if (options.to === 'repaired') {
    if (options.actor.kind !== 'executor') throw new Error('Only an executor can record repair evidence.');
    ...
    if (options.evidence.length === 0) throw new Error('Repair transitions require linked implementation or check evidence.');
  }
  if (options.to === 'independently_verified') {
    if (options.actor.kind !== 'verifier') throw new Error('Only a read-only verifier can independently verify a finding.');
    if (options.finding.state !== 'repaired') throw new Error('Independent verification requires a repaired finding.');
    if (options.evidence.length === 0) throw new Error('Independent verification requires observable evidence.');
  }
...
export function markFindingsStale(options: {
  ...
  return options.findings.map((finding) => {
    if (!changed.has(finding.scope.identity) && ![...changed].some((scope) =>
      finding.scope.identity.startsWith(`${scope}:`) || scope.startsWith(`${finding.scope.identity}:`))) {
      return finding;
    }
    if (finding.state === 'open' || finding.state === 'human_needed' || finding.state === 'stale') return finding;
