...
import { TaskNodeV1Schema, type TaskNodeV1 } from './schemas.js';
...
function normalizeWritePath(value: string): string {
...
export function buildExecutionGraph(input: TaskNodeV1[]): ExecutionGraphV1 {
  ...
  if (byId.size !== nodes.length) throw new Error('Execution graph contains duplicate task IDs.');
  for (const node of nodes) {
    for (const dependency of node.dependencies) {
      if (!byId.has(dependency)) {
        throw new Error(`Task '${node.taskId}' depends on unknown task '${dependency}'.`);
      }
      if (dependency === node.taskId) throw new Error(`Task '${node.taskId}' depends on itself.`);
    }
  }
  ...
  while (completed.size < nodes.length) {
    ...
    for (const node of wave) completed.add(node.taskId);
  }
  return { nodes, waves };
}

export function portableWriteSet(values: string[], pathApi: path.PlatformPath = path): string[] {
