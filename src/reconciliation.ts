...
function ids(artifacts: RelayRunV1['artifacts'], marker: 'requirement' | 'scenario'): string[] {
...
export function reconcileCompiledOpenSpec(options: {
  ...
  const changedArtifactPaths = [...new Set([...oldDigests.keys(), ...currentDigests.keys()])]
    ...
    .sort();
  const changedRequirementIds = symmetricDifference(
    ids(options.run.artifacts, 'requirement'),
    options.compiled.requirementIds,
  );
  const changedScenarioIds = symmetricDifference(
    ids(options.run.artifacts, 'scenario'),
    options.compiled.scenarioIds,
  );
  ...
  for (const evidence of options.assurance.evidence) {
    if (evidence.taskId) {
      ...
      if (!task) {
        ...
        throw new Error(
          `Evidence '${evidence.evidenceId}' cannot bind to task '${task.taskId}' because it lacks ` +
          'an explicit stable identifier.',
        );
      }
    }
    if (Object.entries(evidence.sourceDigests ?? {}).some(
  ...
  const reconciliation: SourceReconciliationV1 = {
    addedTaskIds,
    removedTaskIds,
