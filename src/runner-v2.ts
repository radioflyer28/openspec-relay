...
function legacyConfig(config: RelayConfigV2) {
...
async function refreshPlanningEvents(options: {
  ...
  compiled: Awaited<ReturnType<typeof compileOpenSpecChange>>;
  current: ReturnType<typeof replayRelayEventsV2>;
  ...
  uatScenarios: RelayAssuranceV2['uatScenarios'];
  ...
  const append = async (eventId: string, actor: Parameters<typeof createRelayEventV2>[0]['actor'],
    payload: Parameters<typeof createRelayEventV2>[0]['payload']) => appendRelayEventV2({
    changeDir: options.changeDir,
